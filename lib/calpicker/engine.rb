...
module Calpicker
  ...
  # Returns the default_options hash.  These options are by default provided to every calpicker control, unless otherwise overrided.
  # 
  # Example:
  #   # At the bottom of config/environment.rb:
  #   Calpicker.default_options.update(
  #     :popup => "force",
  #     :month_year => "label",
  #     :image => "custom_calendar_picker.png"
  #   )
  def self.default_options
    @calpicker_default_options ||= { :image => "calpicker/calendar.gif" }
  end
  ...
  #
  # Example:
  #   Calpicker.format = :italian
  #   puts Calpicker.format[:date]
  #     => "%d/%m/%Y"
  def self.format
    @calpicker_format ||= FORMATS[:natural]
  end

  # Set the format.  To see a list of available formats, Calpicker::FORMATS.keys, or open calpicker/calpicker.rb
  #
  # (e.g. Calpicker.format = :italian)
  def self.format=(format)
    raise "Calpicker: Unrecognized format specification: #{format}" unless FORMATS.has_key?(format)
    @calpicker_format = FORMATS[format]
  end
  ...
  class Engine < ::Rails::Engine
    isolate_namespace Calpicker
    initializer "calpicker.assets.precompile" do |app|
      app.config.assets.precompile += %w(default.css silver.css)
    end
    initializer 'my_engine.action_controller' do |app|
      ActiveSupport.on_load :action_controller do
        helper Calpicker::FormHelpers
        helper Calpicker::IncludesHelper
      end
    end
  end
end
