...
module Calpicker::FormHelpers
  ...
  #   +value+ - When specified as a string, uses value verbatim.  When Date, DateTime, Time, it converts it to a string basd off the format set by Calpicker#format=
  #   +options+ - ...
  #
  # == Options
  # 
  ...
  # 
  #   <%= calpicker_tag "name", "2007-01-01", :embedded => true %>
  # 
  # === :hidden
  # 
  ...
  # 
  #   <span id='cds_value' /> 
  #   <%= calpicker_tag "hidden_date_selector", "", :hidden => "true", :onchange => "$('cds_value').update($F(this));" %>
  # 
  # === :image
  # 
  ...
  # 
  #   <%= calpicker_tag "altered_image", "", :image => "groovy.png" %>
  # 
  # === :minute_interval
  # 
  # Specifies the minute interval used in the hour/minute selector.  Default is 5.
  # 
  #   <%= calpicker_tag "month_year_selector_label", "", :minute_interval => 15 %>
  # 
  # === :month_year
  # 
  ...
  # 
  #    <%= calpicker_tag "month_year_selector_label", "", :month_year => "label" %>
  # 
  # === :popup => 'force'
  # 
  # Forces the user to use the popup calendar by making it's text-box read-only and causing calpicker to override it's default behavior of not allowing selection of a date on a target element that is read-only.
  # 
  #   <%= calpicker_tag "name", "2007-01-01", :popup => "force" %>
  # 
  # === :time
  # 
  ...
  # 
  #   <%= calpicker_tag "e_date", nil, :year_range => 10.years.ago..0.years.from_now %>
  #   <%= calpicker_tag "e_date", nil, :year_range => [0.years.ago, 10.years.from_now] %>
  #   <%= calpicker_tag "e_date", nil, :year_range => 2000..2007 %>
  #   <%= calpicker_tag "e_date", nil, :year_range => [2000, 2007] %>
  # 
  # == CALLBACKS
  # 
  ...
  # }}}
  # 
  # All callbacks are executed within the context of the target input element.  If you'd like to access the Calpicker object itself, you can access it via "this.calpicker".
  # 
  # For example:
  # 
  #   <%= calpicker_tag "event_demo", "", :after_navigate => "alert('The current selected month is ' + this.calpicker.selected_date.getMonth());" ,
  def calpicker_tag( name, value = nil, options = {})
    image, options, javascript_options = calpicker_process_options(options)
    value = Calpicker.format_time(value, javascript_options)
    ...
    calpicker_output(tag, image, options, javascript_options)
  end

  # Similar to the difference between +text_field_tag+ and +text_field+, this method behaves like +text_field+
  #
  # It receives the same options as +calpicker_tag+.  Need for time selection is automatically detected by checking the corresponding column meta information of Model#columns_hash
  def calpicker(object, method, options={})
    ...
    options[:value] ||=
      if(obj.respond_to?(method) && obj.send(method).respond_to?(:strftime))
        obj.send(method).strftime(Calpicker.date_format_string(use_time))
  ...
  private
    ...
    def calpicker_process_options(options)
      options, javascript_options = Calpicker.default_options.merge(options), {}
      ...
      if (default_time = javascript_options[:default_time])
        if default_time.respond_to?(:strftime)
          javascript_options[:default_time] = "new Date('#{default_time.strftime(Calpicker.date_format_string(true))}')"
        else 
    ...
    def calpicker_output(input, image, options = {}, javascript_options = {})
      out = input
...
module ActionView
  module Helpers
    class FormBuilder
      def calpicker(method, options = {})
        @template.calpicker(@object_name, method, options.merge(:object => @object))
      end
    end
  end
end
